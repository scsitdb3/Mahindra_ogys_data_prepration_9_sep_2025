...
state_vars = [
    ...
    "report_results", "show_reports", "oem_mismatches", "mrn_mismatches", "mdarpan_mismatches","intransit_mismatches"
]
for var in state_vars:
    if var not in st.session_state:
        if var in ["validation_errors", "period_validation_errors", "missing_files"]:
            st.session_state[var] = []
        elif var in ["validation_log", "oem_mismatches", "mrn_mismatches", "mdarpan_mismatches","intransit_mismatches"]:
...
def validate_periods(all_locations, start_date, end_date, period_days):
    ...
    for brand, dealer, location, location_path in all_locations:
        ...
        intransit_files = [f for f in os.listdir(location_path) if f.lower().startswith('intransit') or f.lower().startswith('in transit')]   
        if not oem_files or not mrn_files or not intransit_files:
            continue
#oem file validation
...
#intransit file validation
        intransit_periods = {p: False for p in periods}
        for intransit_files in intransit_files:
            try:
                intransit_files = read_file(os.path.join(location_path, intransit_files))
                if intransit_files is None or 'Po Date' not in intransit_files.columns:
                    continue
                elif not oem_file.endswith('.xlsx'):
                    st.warning(f"File not Excel Workbook and .xlsx extention For : {brand}-{dealer}-{location}:-oem_file")
                    continue
                
                intransit_files['Po Date'] = pd.to_datetime(intransit_files['Po Date'], errors='coerce')
                for period_start, period_end in periods:
                    if any(period_start <= d.date() <= period_end for d in intransit_files['Po Date'].dropna()):
                        intransit_periods[(period_start, period_end)] = True
            except Exception as e:
                validation_errors.append(f"{location}: Error validating Intransit periods - {str(e)}")
#mrn file validation
        ...
        for period_start, period_end in periods:
            ...
            if not intransit_periods[(period_start, period_end)]: missing_in.append("Intransit")
            if missing_in:
...
if st.session_state.get("logged_in", False):
    ...
    if (process_btn or st.session_state.continue_processing) and st.session_state.uploaded_file is not None:
        ...
        try:
            ...
            missing_files = []
            for brand, dealer, location, location_path in all_locations:
                required = {'OEM': False, 'MRN': False, 'Stock': False,'Mdarpan':False,'intransit':False}
                for file in os.listdir(location_path):
                    ...
                    if f.startswith('intransit') or f.startswith('in transit'): required['intransit'] = True
            ...
            #st.session_state.intransit_mismatches = intransit_mismatches
