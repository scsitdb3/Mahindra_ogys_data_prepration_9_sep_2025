    ...
    for i, (brand, dealer, location, location_path) in enumerate(all_locations):
        ...
        for file in os.listdir(location_path):
            ...
            if df is not None:
                ...
                elif file_lower.startswith('intransit'):
                    # Validate OEM columns
                    required_cols = ['Po No', 'Part No.', 'Po Date','Quantity']
                    missing_cols = [col for col in required_cols if col not in df.columns]
                    if missing_cols:
                        validation_errors.append(f"{location}: Intransit file missing columns - {', '.join(missing_cols)}")
                    else:
                        Intransit_data.append(df)
        ...
        if oem_data:
            ...
            try:
                ...
                Oem_final_unique_upload_ex = Oem_final_unique_upload.copy()
                if Mdarpan_data:
                ...
                Oem_final_unique_upload_with_intrasit = Oem_final_unique_upload.copy()    
                ...
                if Intransit_data:
                    ...
                    intransit_df.rename(columns={
                        ...
                        'Part No.': 'PartNumber',
                        'Quantity': 'POQty',
                        'Po Date': 'OrderDate'}, inplace=True)
                    intransit_df = intransit_df[['Location', 'PartNumber', 'OrderNumber', 'OrderDate', 'POQty']]
                    intransit_df['OEMInvoiceNo']=''
                    intransit_df['OEMInvoiceDate']=''
                    intransit_df['OEMInvoiceQty']=''    
                    
                    
                    Oem_final_unique_upload_ex['Concat']= Oem_final_unique_upload_ex['Location']+Oem_final_unique_upload_ex['PartNumber'].str.replace(r"['/, ]",'',regex=True) +Oem_final_unique_upload_ex['OrderNumber']
                    intransit_df['Int_Concat']=intransit_df['Location']+intransit_df['PartNumber'].str.replace(r"['/, ]",'',regex=True)+intransit_df['OrderNumber']
                    
                    intransit_mat = intransit_df.merge(Oem_final_unique_upload_ex,left_on='Int_Concat',right_on='Concat',how='left')
                    intransit_mat['Location_y']=intransit_mat['Location_y'].fillna('Indal')
                    intransit_mat_exl = intransit_mat[intransit_mat['Location_y']=='Indal']
                    intransit_mat_exl.rename(columns={'Location_x': 'Location','PartNumber_x': 'PartNumber','OrderNumber_x': 'OrderNumber','OrderDate_x': 'OrderDate',
                    'POQty_x': 'POQty', 'OEMInvoiceNo_x': 'OEMInvoiceNo','OEMInvoiceDate_x': 'OEMInvoiceDate','OEMInvoiceQty_x': 'OEMInvoiceQty'
                    }, inplace=True)
                    intransit_mat_exl_up = intransit_mat_exl[['Location', 'PartNumber', 'OrderNumber', 'OrderDate', 'POQty','OEMInvoiceNo',
                                                            'OEMInvoiceDate','OEMInvoiceQty']]
                   # intransit_mat.to_excel(r'C:\Users\INDAL DAS\Downloads\_intransit_mate.xlsx', index=False)  # For debugging
                         
                #     intransit_mat.to_excel(r'C:\Users\INDAL DAS\Downloads\intransit_mat.xlsx', index=False)  # For debugging
                    Oem_final_unique_upload_with_intrasit = pd.concat([Oem_final_unique_upload, intransit_mat_exl_up], ignore_index=True)


                # ✅ Final assignment
                dfs[key] = Oem_final_unique_upload_with_intrasit
